@extends('layouts.app', ['page' => __('Add School'), 'pageSlug' => 'addSchool', 'pageParent' => 'School'])
        ...
        <div class="col col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="title">{{ _('Add School') }}</h5>
                </div>
                <form method="post" action="{{ route('school.store') }}" autocomplete="off">
                    <div class="card-body">
                            ...
                            <div class="form-group{{ $errors->has('abbrv') ? ' has-danger' : '' }}">
                                ...
                                <input type="text" name="abbrv" class="form-control{{ $errors->has('abbrv') ? ' is-invalid' : '' }}" placeholder="{{ _('Enter school abbrevation') }}" value="{{ old('abbrv', '') }}">
                                @include('alerts.feedback', ['field' => 'abbrv'])
                            </div>

                            <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                <label>{{ _('School Name') }}</label>
                                <input type="text" name="name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ _('Enter school name') }}" value="{{ old('name', '') }}">
                            ...
                            <div class="form-group{{ $errors->has('principal') ? ' has-danger' : '' }}">
                                ...
                                <input type="text" name="principal" class="form-control{{ $errors->has('principal') ? ' is-invalid' : '' }}" placeholder="{{ _('Enter school principal') }}" value="{{ old('principal', '') }}">
